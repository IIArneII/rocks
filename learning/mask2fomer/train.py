...
class CFG:
    coco_json   = Path("data/coco/auto.json")
...
CLASSES = ["rock", "ladle", "ladle_handle"]
...
def build_transforms():
    return A.Compose(
        [
            ...
            A.Rotate(15, border_mode=cv2.BORDER_CONSTANT, p=0.3),
            A.GaussNoise(p=0.25),
            A.HorizontalFlip(p=0.5),
            A.RandomCrop(CFG.img_size, CFG.img_size, p=0.3),
        ]
    )
